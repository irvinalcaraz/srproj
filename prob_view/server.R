...
#   
#   vals = dim(length(value))
#   
#   for(i in 1:length(value)){
#     vals[i] = switch(type,
#                  p = do.call(paste("q",dist,sep=""),c(value[i],params)),
#                  d = value)
#     
#     if(type == "p")
#     {
#       if(value <= 0){
#         vals[i] = do.call(paste("q",dist,sep=""),c(.00001,params))
#       }else if(value >= 1){
#         vals[i] = do.call(paste("q",dist,sep=""),c(.99999,params))
#       }
#     }
#   }
# 
...
xvalue = function(value,type,dist,params){
  ...
  if (!(type == "p" && (value>=1 || value<=0))){
    switch(type,
           p = do.call(paste("q",dist,sep=""),c(value,params)),
           d = value)
  }else if (type == "p" && value>=1 ){
    return(do.call(paste("q",dist,sep=""),c(.99999,params)))
  }else if (type == "p" && value<=0 ){
    return(do.call(paste("q",dist,sep=""),c(.00001,params)))
  }
...
shinyServer(function(input, output, session) {
  ...
  observe({
    if(input$type == "p" &&
       (is.na(input$shadeval1) || input$shadeval1 < 0 || input$shadeval1 > 1) &&
       (is.na(input$shadeval2) || input$shadeval2 < 0 || input$shadeval2 > 1)){
      showshinyalert(session,"probalert",
                     "Please choose a probabability between 0 and 1, exclusive",
                     "danger")
    }
  })
  ...
  output$distPlot <- renderPlot({
    ...
    isolate({
      ...
      if (input$dist == 'beta'
        ...
        if(input$dist == 'unif'){
          if((!is.null(xvalue1))&&(xvalue1<minx)){
            minx = input$p1.unif
            xvalue1 = xvalue(minx,input$type,input$dist,params) 
          }
          
          if((!is.null(xvalue2))&&(xvalue2>maxx)){
            maxx = input$p2.unif
            xvalue2 = xvalue(maxx,input$type,input$dist,params)
          }
        }
      ...
      # title(paste(maxx))
      if (input$type != 'none'){
        # if(input$type == "p" && (xvalue1 < 0 | xvalue1 > 1) )
        ...
        if(input$shade=="left"){
          ...
          mtext(result,3)
          axis(1,at=round(xvalue1,3),pos=0,col.ticks="red",col.axis="red",lwd.ticks = 2,cex.axis=2)
          ...
          mtext(result,3)
          axis(1,at=c(round(xvalue1,3),round(xvalue2,3)),pos=0,col.ticks="red",col.axis="red",lwd.ticks = 2,cex.axis=2)
